...
Private Sub TBBereichVorhandenAktualisieren( _
    ...
    ' Neue Quellblätter werden automatisch als neue Datenspalten
    ' direkt vor dem _NOK-Endmarker angelegt.
    '
    ' Dadurch bleibt die Bereichsstruktur vollständig dynamisch:
    ' Die Anzahl der Dashboard-Datenspalten folgt immer der
    ' aktuellen Anzahl der Quellblätter.
    If anzahlBlaetter > cAnzahlDaten Then

        For i = 1 To anzahlBlaetter - cAnzahlDaten

            wsDash.Columns(cNOK).Insert _
                Shift:=xlToRight, _
                CopyOrigin:=xlFormatFromLeftOrAbove

        Next i

        ' Der _NOK-Marker wurde durch das Einfügen nach rechts
        ' verschoben. Position und Bereichsgrenzen deshalb neu
        ' über den Name Manager ermitteln.
        cNOK = DashboardSpalte( _
            wsDash, _
            ExcelNameBereinigen(bereichsName) & "_NOK")
        ...
        If cNOK <= 0 Then
            ...
            LogEintrag _
                ...
                "TB__ WARNUNG", _
                "", _
                "Bereich '" & bereichsName & "' konnte nach dem " & _
                "automatischen Erweitern nicht erneut über seinen " & _
                "_NOK-Marker gefunden werden."
